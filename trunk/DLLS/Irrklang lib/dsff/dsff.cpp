...
static int isValid(lua_State *Lua)
{
	lua_pop(Lua, 1);
...
static int getNameById(lua_State *Lua)
{
	int var;
	var = (int)lua_tonumber(Lua, -1);
	if (var)
	{
...
{	
	 ...
	 if (typ2 == 1)
	 {
		...
		musice = engine->play3D(name,position,loop,paused,false,ESM_AUTO_DETECT,true);
		if (musice)
		{
...
extern "C" int __declspec(dllexport) luaopen_irrk(lua_State *L)
{
	cout << "Essa DLL é só doidera! Feita por mock airaria. NOOBS" << endl;
	engine = createIrrKlangDevice();
	luaL_register(L, "irrklang", irrk_funcs);
	return 1;
}
